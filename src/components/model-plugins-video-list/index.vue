    ...
    <div class="oh" :style="style_container">
        <div :style="style_img_container">
            <div class="re oh" :style="style">
                <div v-if="!['4'].includes(plugins_video_theme)" class="flex-warp" :class="plugins_video_theme_class" :style="plugins_video_theme != '3' ? plugins_video_spacing : ''">
                    <div v-for="(item, index) in data_list" :key="index" class="item oh" :style="plugins_video_style">
                        <div :class="plugins_video_theme == '0' ? 'flex-row oh' : 'flex-col oh h'" :style="plugins_video_img_style">
                ...
                <div v-else class="oh" :class="plugins_video_theme_class">
                    <swiper :key="carousel_key" class="w flex" direction="horizontal" :loop="true" :autoplay="autoplay" :slides-per-view="Number(carousel_col) + 1" :slides-per-group="slides_per_group" :allow-touch-move="false" :space-between="new_style.plugins_video_spacing" :pause-on-mouse-enter="true" :modules="modules">
                        <swiper-slide v-for="(item, index) in data_list" :key="index">
                            <div class="item oh" :style="plugins_video_style">
                                <div class="h oh flex-col" :style="plugins_video_img_style">
                                    <div class="oh re h">
                                        <template v-if="item.new_cover.length > 0">
                                            <image-empty v-model="item.new_cover[0].url" class="img" :style="img_radius" :error-img-style="error_img"></image-empty>
                                        </template>
                                        ...
                                        <template v-if="field_show.includes('3') && new_content.name_float == '1'">
                                            <div class="text-line-1" :style="plugins_video_name + float_name_style">{{ !isEmpty(item.new_title) ? item.new_title : item.data.title }}</div>
                                        </template>
                                        <!-- 角标设置 -->
                                        <subscript-index :value="props.value"></subscript-index>
                                    </div>
...
// 数据
const data_list = ref<plugins_videoList[]>([]);
// 风格
...
// 浏览量
const plugins_video_page_view = ref('');
// 内容圆角
...
const plugins_video_style = ref({});
const plugins_video_img_style = ref('');
const default_data_list: plugins_videoList = {
    id: 0,
    data: {},
    new_title: '测试视频标题',
    new_cover: [],
...
const carousel_height_computer = computed(() => {
    return new_style.value.plugins_video_height + 'px';
});
...
// 宽度和高度为空的时候，修改默认值
const plugins_video_img_width = computed(() => {
...
// 宽度和高度为空的时候，修改默认值
const plugins_video_img_height = computed(() => {
...
watch(
    ...
    (newVal, oldValue) => {
        ...
        plugins_video_theme.value = new_content.theme;
        ...
        style.value = '';
        // 样式处理
        const all_style = gradient_handle(new_style.plugins_video_color_list, new_style.plugins_video_direction) + margin_computer(new_style.margin) + box_shadow_computer(new_style) + border_computer(new_style);
        if (plugins_video_theme.value == '0') {
            plugins_video_style.value += content_radius.value + all_style;
            plugins_video_img_style.value = content_spacing.value + content_padding.value + background_computer(plugins_video_data);
        } else if (plugins_video_theme.value == '1') {
            plugins_video_style.value += `width: calc(50% - ${(new_style.plugins_video_spacing + plugins_video_left_right_width_margin.value * 2) / 2}px);` + content_radius.value + all_style;
            plugins_video_img_style.value = background_computer(plugins_video_data);
        } else if (plugins_video_theme.value == '2') {
            plugins_video_style.value += content_radius.value + all_style;
            plugins_video_img_style.value = background_computer(plugins_video_data);
        } else if (plugins_video_theme.value == '3') {
            style.value = `padding: 0 ${new_style.content_spacing}px;background:#fff;` + content_radius.value;
        } else if (plugins_video_theme.value == '4') {
            ...
            plugins_video_item_height.value = `${new_style.plugins_video_height}px`;
            plugins_video_style.value += content_radius.value + all_style;
            plugins_video_img_style.value = background_computer(plugins_video_data);
        }
...
.style1 {
    .item {
        .img {
            height: v-bind(plugins_video_img_height);
            width: v-bind(plugins_video_img_width);
        }
    }
}
.style2 {
...
.style5 {
    .item {
        ...
        .title {
            height: v-bind(plugins_video_name_height_computer);
            line-height: v-bind(plugins_video_name_line_height_computer);
        }
    }
}
</style>
