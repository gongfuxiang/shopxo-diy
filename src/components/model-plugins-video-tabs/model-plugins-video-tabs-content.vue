    ...
    <div class="content">
        <el-form :model="form" label-width="74" class="m-h">
            ...
            <card-container>
                ...
                <el-form-item v-if="form.plugins_video_theme == '4'" label="轮播列数">
                    <el-radio-group v-model="form.plugins_video_carousel_col">
                        ...
                        <template #default="{ row, index }">
                            <div class="flex-col w">
                                ...
                                <template v-if="form.tabs_active_index == index">
                                    ...
                                    <data-filter type="plugins_video" :value="row" :list="row.data_list" :base-list="base_list" @add="plugins_video_add(index)" @data_list_replace="data_list_replace($event, index)" @data_list_remove="data_list_remove($event, index)" @data_list_sort="data_list_sort($event, index)"></data-filter>
                                </template>
                            </div>
                        </template>
                    </drag>
        ...
        <url-value-dialog v-model:dialog-visible="url_value_dialog_visible" :type="['plugins_video']" :multiple="url_value_multiple_bool" @update:model-value="url_value_dialog_call_back"></url-value-dialog>
    </div>
</template>
...
onMounted(() => {
    ...
    if ((typeof content_img_width != 'number' || typeof content_img_height != 'number') && !['3', '4'].includes(form.theme)) {
        const list = base_list.plugins_video_theme_list.filter(item => item.value == form.theme);
        if (list.length > 0) {
...
const plugins_video_theme_change = (val: any) => {
    if (val == '3' || val == '4') {
        form.field_show = ['1', '3'];
    } else {
    ...
    const list = base_list.plugins_video_theme_list.filter(item => item.value == val);
    if (list.length > 0) {
        // emits('theme_change', list[0].width, list[0].height);
...
const data_list_replace = (index: number, row_index: number) => {
    plugins_video_index.value = row_index;
...
const plugins_video_add = (index: number) => {
    ...
    url_value_dialog_visible.value = true;
    plugins_video_index.value = index;
};
// 添加回调
const url_value_dialog_call_back = (item: any[]) => {
    if (url_value_multiple_bool.value) {
        item.forEach((child: any) => {
            form.tabs_list[plugins_video_index.value].data_list.push({
                id: get_math(),
                new_title: '',
                new_cover: [],
                data: child,
        ...
        form.tabs_list[plugins_video_index.value].data_list[data_list_replace_index.value] = {
            id: get_math(),
            new_title: '',
            new_cover: form.tabs_list[plugins_video_index.value].data_list[data_list_replace_index.value]?.new_cover || [],
            data: item[0],
        };
    }
};
