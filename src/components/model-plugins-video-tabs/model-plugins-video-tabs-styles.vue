        ...
        <el-form :model="form" label-width="74">
            ...
            <card-container>
                ...
                <el-form-item label="内容背景">
                    <background-common v-model:color_list="form.plugins_video_content_color_list" v-model:direction="form.plugins_video_content_direction" v-model:img_style="form.plugins_video_content_background_img_style" v-model:img="form.plugins_video_content_background_img" @mult_color_picker_event="plugins_video_content_mult_color_picker_event" />
                </el-form-item>
                <el-form-item label="圆角">
                    <radius :value="form.plugins_video_content_radius"></radius>
                </el-form-item>
                <el-form-item label="外间距">
                    <margin :value="form.plugins_video_content_margin"></margin>
                </el-form-item>
                <el-form-item label="内间距">
                    <padding :value="form.plugins_video_content_padding"></padding>
                </el-form-item>
                <el-form-item label="数据间距">
                    <slider v-model="form.plugins_video_content_spacing" :max="100"></slider>
                </el-form-item>
                <!-- 边框处理 -->
                <border-config v-model:show="form.plugins_video_content.border_is_show" v-model:color="form.plugins_video_content.border_color" v-model:style="form.plugins_video_content.border_style" v-model:size="form.plugins_video_content.border_size"></border-config>
                <!-- 阴影配置 -->
                <shadow-config v-model="form.plugins_video_content"></shadow-config>
            </card-container>
            <div class="divider-line"></div>
            <card-container>
                <div class="mb-12">视频样式</div>
                <el-form-item v-if="theme != '3'" label="视频背景">
                    <background-common v-model:color_list="form.plugins_video_color_list" v-model:direction="form.plugins_video_direction" v-model:img_style="form.plugins_video_background_img_style" v-model:img="form.plugins_video_background_img" @mult_color_picker_event="mult_color_picker_event" />
                </el-form-item>
                <el-form-item label="视频名称">
                    <color-text-size-group v-model:color="form.name_color" v-model:typeface="form.name_weight" v-model:size="form.name_size" :label-width="data.plugins_video_theme == '4' && data.name_float == '1' ? 60 : 40">
                        <template v-if="data.plugins_video_theme == '4' && data.name_float == '1'">
                ...
                <template v-if="theme != '3'">
                    <el-form-item label="视频间距">
                        <slider v-model="form.plugins_video_spacing"></slider>
                    </el-form-item>
                </template>
                ...
                <template v-if="theme == '4'">
                    <el-form-item label="内容高度">
                        <slider v-model="form.plugins_video_height" :max="1000"></slider>
                    </el-form-item>
                </template>
                <template v-if="theme != '3'">
...
// 主题
const theme = computed(() => data.value.plugins_video_theme);
// 兼容旧数据
if (theme.value == '0') {
...
// 内容区域背景渐变设置
const plugins_video_content_mult_color_picker_event = (arry: color_list[], type: number) => {
    form.value.plugins_video_content_color_list = arry;
    form.value.plugins_video_content_direction = type.toString();
};
// 视频背景渐变设置
const mult_color_picker_event = (arry: color_list[], type: number) => {
    form.value.plugins_video_color_list = arry;
    form.value.plugins_video_direction = type.toString();
};
// 通用样式回调
