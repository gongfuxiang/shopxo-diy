        ...
        <div class="flex-col" :style="style_img_container">
            <div class="oh" :style="tabs_container">
                <div class="oh" :style="tabs_img_container">
                    <tabs-view ref="tabs" :value="plugins_video_tabs" :active-index="tabs_active_index" :tabs-sliding-fixed-bg="tabs_sliding_fixed_bg"></tabs-view>
                </div>
            </div>
            <div class="oh" :style="plugins_video_container">
                <div class="oh" :style="plugins_video_img_container">
                    <model-plugins-video-list :value="plugins_video_tabs" :is-common-style="false"></model-plugins-video-list>
                </div>
            </div>
        </div>
    </div>
...
const plugins_video_tabs = ref({});
const tabs_active_index = ref(0);
// 选项卡独立样式
const tabs_container = ref('');
const tabs_img_container = ref('');
// 区域内边距
const plugins_video_container = ref('');
const plugins_video_img_container = ref('');
// 打开滑动固定开关之后，显示的样式
...
watch(
    ...
    (val) => {
        ...
        const plugins_video_content_data = {
            ...
            background_img_style: new_style.plugins_video_content_background_img_style,
            background_img: new_style.plugins_video_content_background_img,
        }
        ...
        plugins_video_tabs.value = new_data;

        style_container.value = common_styles_computer(new_style.common_style);
        style_img_container.value = common_img_computer(new_style.common_style) + `gap: ${new_style.plugins_video_content_spacing}px;`;
    },
